...
function setupNavigation() {
    ...
    const titles = {
        "dashboard-section": {
            ...
            subtitle: "Analyze maize leaves for crop nutrient deficiencies using Machine Learning"
        },
        "guide-section": {
...
function setupImageUpload() {
    ...
    const leafWarning = document.getElementById("leaf-warning-banner");
    const leafWarningDismiss = document.getElementById("leaf-warning-dismiss");
    ...
    leafWarningDismiss.addEventListener("click", () => {
    ...
    function handleFile(file) {
        ...
        currentFile = file;
        leafWarning.classList.add("hidden"); // Reset warning on new file
        ...
        reader.onload = (e) => {
            ...
            previewImg.onload = () => {
                ...
                // --- LAYER 1: Client-side green-leaf heuristic ---
                const isLeaf = validateGreenLeaf(previewImg);
                if (!isLeaf) {
                    leafWarning.classList.remove("hidden");
                    writeConsole("[cv] Warning: uploaded image does not appear to be a leaf (low green-channel ratio).");
                } else {
                    leafWarning.classList.add("hidden");
                    writeConsole("[cv] Leaf heuristic passed — green channel ratio within expected range.");
                }
            };
        };
    ...
    function resetUpload() {
        ...
        document.getElementById("results-rejected").classList.add("hidden");
...
// ==========================================================================
// MAIZE LEAF VALIDATION HEURISTIC (Layer 1 — Client-Side)
// ==========================================================================
/**
 * Checks whether an image contains a significant proportion of
 * leaf-like pixels (green, yellow, purple, pale-green — typical of maize leaves).
 * Returns true if the image likely is a maize leaf, false otherwise.
 */
function validateGreenLeaf(imgElement) {
    const canvas = document.createElement("canvas");
    const ctx = canvas.getContext("2d");
    const size = 96; // Small resolution for speed
    canvas.width = size;
    canvas.height = size;
    ...
    for (let i = 0; i < data.length; i += 4) {
        ...
        if (brightness > 40) { // Ignore near-black background pixels
            brightPixels++;

            // Green-dominant (healthy or lightly deficient leaf)
            const isGreen = g > r && g > b && g > 60;
            // Yellowed leaf (nitrogen deficiency: high R+G, low B)
            const isYellow = r > 110 && g > 100 && b < 100 && r > b;
            // Purple/anthocyanin tones (phosphorus deficiency)
            const isPurple = r > 80 && b > 80 && g < r && g < b;
            // Pale/chlorotic yellow-green (potassium deficiency)
            const isPaleGreen = g > 80 && r > 60 && b < 90 && g >= r;

            if (isGreen || isYellow || isPurple || isPaleGreen) {
                leafPixels++;
            }
        }
    }

    if (brightPixels === 0) return false;
    const leafRatio = leafPixels / brightPixels;
    // Require at least 18% of non-background pixels to be leaf-like colors
    return leafRatio >= 0.18;
}
...
async function triggerAnalysis() {
    ...
    const resultsRejected = document.getElementById("results-rejected");
    ...
    resultsRejected.classList.add("hidden");
    ...
    try {
        if (apiOnline) {
            ...
            // --- LAYER 2: Backend confidence gate ---
            if (response.status === 422) {
                // Server rejected image as non-maize (low confidence)
                clearInterval(loadingInterval);
                scanOverlay.classList.add("hidden");
                resultsLoading.classList.add("hidden");
                resultsRejected.classList.remove("hidden");
                btnAnalyze.removeAttribute("disabled");
                writeConsole("[model] Rejection: model confidence too low — image does not appear to be a maize leaf.");
                return;
            }
